...
公开演示与正式产品现在使用同一套纵向树、节点卡片、详情面板、进度状态和依赖线层级

下面的图片由正式产品仓库内的双语公开案例页直接生成；普通前置关系使用清晰的深灰实线；经过当前节点的路径使用更粗的棕色线

图库覆盖四个领域、四个树位置和四种工作界面；不包含个人进度、私有地址、访问控制配置、部署细节或私有规模统计
